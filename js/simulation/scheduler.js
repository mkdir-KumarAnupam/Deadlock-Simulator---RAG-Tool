        ...
        requests.forEach(req => {
          ...
          if (allocatedCount < resource.capacity) {
            ...
            if (reqIndex > -1) {
              ...
              if (window.pushNarratorEvent) pushNarratorEvent('allocate', { resource: resource.label, label: process.label });
        ...
        if (currentRunningNodeId) {
          ...
          if (p && p.state === 'RUNNING') {
            ...
            // Round Robin: Check quantum expiry
              ...
              if (window.pushNarratorEvent) pushNarratorEvent('preempt', { label: p.label, by: 'quantum expiry' });
            }
            // SRTF: Check for preemption
              ...
              if (shorterProcess) {
                ...
                if (window.pushNarratorEvent) pushNarratorEvent('preempt', { label: p.label, by: shorterProcess.label });
              }
            }
            // Priority (Preemptive): Check for preemption
              ...
              if (higherPriorityProcess) {
                ...
                if (window.pushNarratorEvent) pushNarratorEvent('preempt', { label: p.label, by: higherPriorityProcess.label });
              }
            }
        ...
        if (!currentRunningNodeId) {
          ...
          if (candidates.length > 0) {
            ...
            if (nextP) {
              ...
              activeColor = config.colors.running;
              if (window.pushNarratorEvent) pushNarratorEvent('schedule', { label: nextP.label });
      ...
      function terminateProcess(p) {
        ...
        if (window.pushNarratorEvent) pushNarratorEvent('terminate', { label: p.label });
      ...
      function updateGantt(label, color) {
        ...
        // If switching from one process to another
